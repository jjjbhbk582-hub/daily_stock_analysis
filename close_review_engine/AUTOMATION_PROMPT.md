...
close_review_engine/data/state/trade_plans.json
close_review_engine/data/state/trade_outcomes.jsonl
...
4. 报告必须保留十一部分：市场环境；行业板块完整排名；重点概念板块；强势/上升/退潮板块；重点板块2+2；59只固定股票完整排名；固定池Top5；动态候选买点；与上一次比较；推荐交易计划；最终操作结论。推荐交易计划必须区分今日可执行、等待触发、技术交易但基本面缺失、观察和回避；分别列出回踩与突破的入场、量化触发、禁止追高、独立止损、两档止盈、两档赔率、模型仓位上限和失效条件；并逐股说明多周期趋势、量价指标、基本面、板块强弱、位置赔率、触发条件和未通过门槛的原因。无股票通过绝对门槛时必须明确“今日无可执行交易”，不得从相对排名中凑数。
...
11. 推送前先验收trade_plans.json中的上一交易日活动计划，遵守T+1、跳空取消、目标1减半和歧义排除；结果只追加到trade_outcomes.jsonl。状态文件损坏时保留原文件并明确报警，不得伪造统计。
12. 基本面missing/stale/partial不直接隐藏技术机会，但必须显示缺失字段；missing/stale单股模型仓位上限7.5%、partial 10%、verified 15%，且技术交易不得写成综合推荐。
13. 滚动统计只读取真实保存的计划；少于30笔时原样保留“统计置信度不足”。
```
