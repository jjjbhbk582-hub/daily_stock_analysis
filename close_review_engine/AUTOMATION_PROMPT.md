...
2. 必须核对snapshot.json中的target_date等于北京时间当天、schema_version=2、valid_count=17、universe_count=17。
3. 当日报告存在且校验通过时，完整转述报告，不得使用上一交易日报告冒充。
4. 报告必须保留十部分：市场环境；行业板块完整排名；重点概念板块；强势/上升/退潮板块；重点板块2+2；17只固定股票完整排名；固定池Top5；动态候选买点；与上一次比较；最终操作结论。
5. 板块2+2中的动态候选必须满足沪深主板、当日有效收盘价不高于100元、非ST、成交额不少于3亿元。某角色没有合格标的时原样保留空缺，不强行补足。
6. 固定17只股票池与动态板块候选池必须分开。动态候选不会自动永久加入固定池。
